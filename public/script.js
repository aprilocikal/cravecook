...
function filterAndRender() {
    ...
    document.getElementById('tableCount').innerText = `${filteredRecipes.length} recipes`;
    renderTable();
    renderPagination();
}
...
function openModal(mode, recipeObj = null) {
    ...
    const inputs = ['recipe_name', 'prep_time', 'cook_time', 'total_time', 'servings', 'rating', 'img_src'];
    ...
    if (mode === 'add') {
        ...
        inputs.forEach(id => document.getElementById(id).value = recipeObj[id] || '');
...
async function loadAnalytics() {
    try {
        ...
        // Stat cards
        document.getElementById('aAvg').innerText = data.avgResult.avg ? data.avgResult.avg.toFixed(2) : '—';
        ...
        const labels = { 1:'1–2', 2:'2–3', 3:'3–4', 4:'4–4.5', 4.5:'4.5–5', 5:'5.0' };
        const colors = ['cyan','yellow','yellow','green','green','green'];
        document.getElementById('ratingChart').innerHTML = data.ratingDist.map((r, i) => `
            <div class="bar-row">
        ...
        document.getElementById('cuisineChart').innerHTML = data.cuisines.map((c, i) => `
            <div class="bar-row">
        ...
        const sLabels = { 1:'1–4', 5:'5–9', 10:'10–19', 20:'20–49', 50:'50–99', '100+':'100+' };
        document.getElementById('servingsChart').innerHTML = data.servings.map(s => `
            <div class="bar-row">
